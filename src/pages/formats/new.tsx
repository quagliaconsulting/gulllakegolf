...
const NewFormatPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post('/api/formats', formData, authHeader);
      router.push('/formats');
    } catch (err: any) {
      setError(err.response?.data?.error || 'Failed to create format');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">Add New Format</h1>
