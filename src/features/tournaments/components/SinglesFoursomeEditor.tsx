...
import { MatchService, PlayerMatchup } from '@/services/match/matchService';
import { useApi } from '@/services/api/apiClient';

interface SinglesFoursomeEditorProps {
  tournamentId?: string; // Made optional since BatchAssignPage doesn't pass it
  homeTeamId: string;
  awayTeamId: string;
  homeTeamName?: string; // Made optional since BatchAssignPage doesn't pass it
  awayTeamName?: string; // Made optional since BatchAssignPage doesn't pass it
  formatId: string;
  scheduleId: string;
  courseId: string;
  startingHole: number;
  teeTime: any; // More flexible type to accommodate different date formats
  homePlayers?: any[]; // Added to support pre-populated players
  awayPlayers?: any[]; // Added to support pre-populated players
  onCancel: () => void;
  onSubmit?: (groups: any[]) => Promise<void>; // Added to match BatchAssignPage
  onSave?: () => void; // Made optional for backward compatibility
}
...
export default function SinglesFoursomeEditor({
  ...
  // Use API client with SWR for data fetching
  const { 
    data: homeTeamData,
    error: homeTeamError,
    isLoading: homeTeamLoading
  } = useApi(
    homeTeamId ? `/api/teams/${homeTeamId}` : null,
  ...
  const { 
    ...
    awayTeamId ? `/api/teams/${awayTeamId}` : null,
    { revalidateOnFocus: false }
  );

  // Process API data
  useEffect(() => {
    // If we have pre-populated players (from batch assign), use them
    if (initialHomePlayers?.length) {
      setHomePlayers(initialHomePlayers);
    } else if (homeTeamData?.players) {
      setHomePlayers(homeTeamData.players);
    }

    if (initialAwayPlayers?.length) {
      setAwayPlayers(initialAwayPlayers);
    } else if (awayTeamData?.players) {
      setAwayPlayers(awayTeamData.players);
    }

    // Data loading is complete when we have both teams' players
    if (
      (!homeTeamLoading && !awayTeamLoading) || 
      (initialHomePlayers?.length && initialAwayPlayers?.length)
    ) {
      setLoading(false);
    }
  }, [
  ...
  // Save the foursome using the MatchService
  const handleSave = async () => {
    try {
      ...
      if (onSubmit) {
        // For batch assign page
        await onSubmit(matchups);
      } else {
        // Direct API call using MatchService
        const matchService = new MatchService();
        await matchService.createFoursome({
